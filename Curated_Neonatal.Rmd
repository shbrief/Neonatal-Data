...
colnames(curated_delivery_dat) <-
    c("curation_id",
      "original_neonatal_delivery_procedure_source",
      "original_neonatal_delivery_procedure_value",
...
```{r}
non_na_ind <- which(!is.na(curated_delivery_dat$original_neonatal_delivery_procedure_source))
...
sum(tb$premature == "yes" & tb$gestational_age > 37, na.rm = TRUE)
```
...
```{r}
## Update column names
colnames(curated_premature_dat) <- 
    c("curation_id",
      "original_neonatal_preterm_birth_source",
      "original_neonatal_preterm_birth_value",
      "curated_neonatal_preterm_birth",
      "curated_neonatal_preterm_birth_ontology_term_id")
```

```{r}
## Check the harmonized/curated metadata
non_na_ind <- which(!is.na(curated_premature_dat$original_neonatal_preterm_birth_source))
...
colnames(curated_feeding_dat) <-
    c("curation_id",
      "original_neonatal_feeding_method_source",
      "original_neonatal_feeding_method_value",
      "curated_neonatal_feeding_method")
```

```{r}
## Check the harmonized/curated metadata
non_na_ind <- which(!is.na(curated_feeding_dat$original_neonatal_feeding_method_source))
...
readr::write_csv(curated_feeding_dat2, "data/curated_feeding_method.csv")
```